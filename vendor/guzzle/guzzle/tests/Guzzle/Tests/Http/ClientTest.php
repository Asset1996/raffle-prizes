...
class ClientTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    {
        $client = new Client();
        $request = $client->createRequest('GET', 'http://www.foo.com', array('UserModel-agent' => 'foo'));
        $this->assertEquals('foo', (string) $request->getHeader('UserModel-Agent'));
    }
    ...
    public function testUsesDefaultUserAgent()
    {
        ...
        $this->assertContains('Guzzle/', (string) $request->getHeader('UserModel-Agent'));
    }
